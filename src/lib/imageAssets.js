...
function historyDraftTopicCandidates({ jobDir, historyItem = {}, agentResult = {} } = {}) {
  const candidates = [
    historyItem.topic,
    historyItem.title,
    agentResult.topic,
  ...
  for (const fileName of promptFiles) {
    ...
    try {
      ...
      const match = raw.match(/^\s*Topic:\s*(.+?)\s*$/im);
      if (match) candidates.push(match[1]);
    } catch {
      // Best effort only. A missing prompt must not prevent history recovery.
    }
  }

  const sourceMaterialPath = path.join(jobDir, "source-material.json");
  try {
    if (fs.existsSync(sourceMaterialPath)) {
      const sourceMaterial = JSON.parse(fs.readFileSync(sourceMaterialPath, "utf8").replace(/^\uFEFF/, ""));
      for (const document of Array.isArray(sourceMaterial?.documents) ? sourceMaterial.documents : []) {
        candidates.push(document?.title);
        const filename = String(document?.filename || "").trim();
        if (filename) candidates.push(filename.replace(/\.[^.]+$/, ""));
      }
    }
  } catch {
    // Best effort only. The article and history entry remain usable without source metadata.
  }
...
function resolveExistingHistoryImage(runtimeRoot, jobDir, rawPath) {
  const value = String(rawPath || "").trim();
  if (!value) return "";
  ...
  const candidates = path.isAbsolute(value)
    ? [value]
    : [
      path.resolve(jobDir, value),
      path.resolve(runtimeRoot, value),
      path.resolve(imageRoot, value),
      path.join(imageRoot, path.basename(value))
    ];
  for (const candidate of candidates) {
    try {
      if (fs.existsSync(candidate) && fs.statSync(candidate).isFile() && isImageFile(candidate)) {
        return candidate;
      }
    } catch {
...
function findStoredHistoryImage(runtimeRoot, topicCandidates, suffix) {
  const imageRoot = path.join(runtimeRoot, "image");
  const extensions = [".png", ".jpg", ".jpeg", ".webp"];
  for (const topic of topicCandidates) {
    const base = safeTopicName(topic);
    for (const extension of extensions) {
      const candidate = path.join(imageRoot, `${base}_${suffix}${extension}`);
      try {
        if (fs.existsSync(candidate) && fs.statSync(candidate).isFile()) return candidate;
      } catch {
        // Try the next extension or topic.
      }
    }
  }
  return "";
...
function normalizeHistoryDraftAssets({ runtimeRoot, jobDir, historyItem = {}, agentResult = {} } = {}) {
  const topicCandidates = historyDraftTopicCandidates({ jobDir, historyItem, agentResult });
  const warnings = [];
  let titleImagePath = resolveExistingHistoryImage(runtimeRoot, jobDir, agentResult.titleImagePath);
  if (!titleImagePath) {
    titleImagePath = findStoredHistoryImage(runtimeRoot, topicCandidates, "img_title");
  }
  if (String(agentResult.titleImagePath || "").trim() && !titleImagePath) {
    warnings.push("이력의 타이틀 이미지 경로를 찾지 못했습니다. 이미지 없이 발행할 수 있습니다.");
  }

  const bodyImages = [];
  for (const item of Array.isArray(agentResult.bodyImages) ? agentResult.bodyImages : []) {
    const sequence = Number(item?.sequence);
    if (!Number.isFinite(sequence) || sequence <= 0) continue;
    const resolvedPath = resolveExistingHistoryImage(runtimeRoot, jobDir, item?.path)
      || findStoredHistoryImage(runtimeRoot, topicCandidates, `img_${sequence}`);
    if (!resolvedPath) {
      warnings.push(`이력의 본문 이미지 ${sequence} 경로를 찾지 못했습니다. 해당 이미지는 건너뜁니다.`);
      continue;
    }
    bodyImages.push({ ...item, sequence, path: resolvedPath });
  }

  return { titleImagePath, bodyImages, warnings, topicCandidates };
}
