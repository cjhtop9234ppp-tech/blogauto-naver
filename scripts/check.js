...
const longPdfSections = Array.from({ length: 120 }, (_item, index) => ({
  heading: `페이지 ${index + 1}`,
  text: index === 96 ? "차량세팅 마루식 사전계측 일반 AOS 2017 조정시간 결정금액 0.60H" : "관련 없는 일반 연구 내용"
}));
const pairedSourcePrompt = imageContractPrivate.compactSourceDocumentsForPrompt([
  {
    filename: "원본.pdf",
    file_type: "pdf",
    title: "표준작업시간 연구보고서",
    text: "",
    source_sections: longPdfSections,
    tables: [],
    metadata: { pageCount: 264 }
  },
  {
    filename: "2-2_외판패널_판금작업시간_강의용_블로그용.md",
    file_type: "md",
    title: "외판패널 정리",
    text: "차량세팅과 사전계측을 정리한 보조 설명",
    source_sections: [{ heading: "요약", text: "차량세팅과 사전계측" }],
    tables: [],
    metadata: {}
  }
], [], 20000, "차량세팅 마루식 사전계측 일반 AOS 2017 조정시간 결정금액");
if (!pairedSourcePrompt.includes("최우선 원본(PDF)") || !pairedSourcePrompt.includes("보조 정리(MD)") || !pairedSourcePrompt.includes("페이지 97") || !pairedSourcePrompt.includes("PDF가 포함된 경우 PDF는 최우선 원본 근거")) {
  failed = true;
  console.error("src/lib/codexRunner.js: paired PDF/MD source roles and focused sections were not included in the prompt");
}
...
if (!compactResearchRetryPrompt.includes("사용자 복구 지시 테스트") || !compactResearchRetryPrompt.includes("User-provided repair instruction")) {
  failed = true;
  console.error("src/lib/codexRunner.js: Research resume prompt must be passed explicitly to the retry prompt");
}
...
assertCondition(
  sourceFiles.rendererStyles.content.includes("grid-template-rows: minmax(0, 0.9fr) minmax(0, 1.15fr) minmax(0, 0.8fr);"),
  "src/renderer/styles.css: center column and three Agent log rows must have explicit shrinkable heights"
);
...
if (
  codexGeneration
  && (!codexGeneration.content.includes("safeFallbackBlockRewrite")
    || !codexGeneration.content.includes("mainReviewStatus === \"REVISION\" || mainReviewPassIssue || safeFallbackBlockRewrite")
    || !codexGeneration.content.includes("attempt < maxReviewAttempts"))
) {
  failed = true;
  console.error("src/lib/codexRunner.js: REVISION or safe-rewriteable BLOCK Main Agent review must retry before blocking publishing");
}
...
// The large publisher contains nested template expressions that the simple
// brace scanner intentionally does not parse. Its regression checks only need
// the publisher source from the unique function marker onward.
const publishToNaverMarker = "async function publishToNaver";
const publishToNaverMarkerIndex = sourceFiles.naverPublisher.content.indexOf(publishToNaverMarker);
const publishToNaver = publishToNaverMarkerIndex === -1
  ? null
  : { content: sourceFiles.naverPublisher.content.slice(publishToNaverMarkerIndex) };
if (!publishToNaver) {
  failed = true;
  console.error("src/lib/naverPublisher.js: missing publishToNaver function");
}
...
const fallbackExcerpt = "자동차산업과 소프트웨어 경쟁의 구조를 설명하는 충분한 참고 내용입니다. 검색 후보에서 확인된 맥락을 바탕으로 일반적인 설명을 작성할 수 있습니다. 기업과 정책 환경의 관계, 산업 변화의 방향, 독자가 구분해야 할 사실과 해석을 함께 확인합니다.";
const fallbackOptions = {
  sourceMode: "research",
  forceWebSearch: true,
  topic: "빅테크 경쟁 관련 정책",
  searchResults: [1, 2, 3].map((index) => ({
...
const fallbackResearch = {
  status: "REVISION",
  finalTitle: "빅테크 경쟁 관련 정책, 자동차산업 연결성을 확인할 규제 근거",
  failureReason: "구체적 근거 부족"
};
if (!imageContractPrivate.isSafeGeneralTopicFallbackEligible(fallbackOptions, fallbackResearch)) {
  failed = true;
  console.error("src/lib/codexRunner.js: broad research fallback eligibility regression");
} else {
  const fallbackResult = imageContractPrivate.buildSafeGeneralTopicFallback(fallbackOptions, fallbackResearch);
  if (!fallbackResult || fallbackResult.status !== "PASS" || fallbackResult.safeGeneralTopicFallback !== true || fallbackResult.currentBridgeRequired !== false) {
    failed = true;
    console.error("src/lib/codexRunner.js: broad research fallback result regression");
  }
}

const dailyTopicPlan = createRandomResearchPlan({
  date: "2026-09-13",
  accountId: "check-account",
  blogId: "check-blog",
  category: {
    name: "알아두면 좋은 지식",
    keyword: "AI 기업 동향, 모델 출시, 기술 발표, 투자 소식, 시장 변화, 반도체, 오픈소스 AI, 빅테크 경쟁"
  }
});
const dailyTopicItem = dailyTopicPlan.items.find((item) => item.selectionState === "selected");
if (!dailyTopicItem || dailyTopicPlan.topicSelectionValidation?.status !== "PASS") {
  failed = true;
  console.error("src/lib/dailyWorkflow.js: random topic preflight must select a validated candidate");
}
if (dailyTopicItem && /빅테크.*보상|빅테크.*신모델/.test(dailyTopicItem.topic || "")) {
  failed = true;
  console.error("src/lib/dailyWorkflow.js: random topic preflight must prevent mismatched keyword and slot combinations");
}
const publishApprovedDailyBlock = extractFunctionBlock(sourceFiles.main, "async function publishApprovedDaily", "approved daily publishing");
const randomDailyResearchBlock = extractFunctionBlock(sourceFiles.main, "async function runRandomDailyResearch", "random daily research");
const analyzeSelectedHistoryBlock = extractFunctionBlock(sourceFiles.rendererApp, "async function analyzeAndRepublishSelectedHistory", "bulk history diagnosis and republish");
const publishHistoryDraftBlock = extractFunctionBlock(sourceFiles.main, "async function publishHistoryDraft", "history draft publishing");
const publishHistoryDraftsBlock = extractFunctionBlock(sourceFiles.main, "async function publishHistoryDrafts", "bulk history draft publishing");
// The renderer handler is an arrow-function listener, so the generic function
// parser (which expects `) {`) cannot safely delimit it. Use the source tail
// from its unique marker for structural assertions instead.
const publishSelectedHistoryMarker = '$("#publishSelectedHistoryButton")?.addEventListener';
const publishSelectedHistoryMarkerIndex = sourceFiles.rendererApp.content.indexOf(publishSelectedHistoryMarker);
const publishSelectedHistoryBlock = publishSelectedHistoryMarkerIndex === -1
 ...
 if (
  ...
  || !sourceFiles.main.content.includes("research-title-search-2-result.json")
  || !sourceFiles.main.content.includes("writer-source-search-before-result-2.json")
  || !sourceFiles.main.content.includes("async function resumeJob")
  || !sourceFiles.main.content.includes('ipcMain.handle("job:resume"')
  || !sourceFiles.main.content.includes("onCheckpoint: (checkpoint = {})")
  || !sourceFiles.main.content.includes("lastCompletedPhase")
  || !sourceFiles.main.content.includes("resumeStateOverride")
  || !sourceFiles.main.content.includes("const savedInput = { ...form }")
) {
  failed = true;
...
);
{
  ...
  const initialCheckpointIndex = sourceFiles.main.content.indexOf("writeJobCheckpoint(runtimeRoot, jobId, {", startJobIndex);
  if (startJobIndex < 0 || resumeStateIndex < 0 || initialCheckpointIndex < 0 || resumeStateIndex > initialCheckpointIndex) {
    failed = true;
    console.error("src/main.js: resume checkpoint must be read before the restart checkpoint is written");
  }
}
if (
  !sourceFiles.codexRunner.content.includes("const checkpoint = (payload = {})")
  || !sourceFiles.codexRunner.content.includes('resumeFrom: "writer"')
  || !sourceFiles.codexRunner.content.includes('resumeFrom: "main_review"')
  || !sourceFiles.codexRunner.content.includes('resumeFrom: "image"')
  || !sourceFiles.codexRunner.content.includes("const isPostResearchResume = Boolean(resumeFrom && resumeFrom !== \"research\")")
  || !sourceFiles.codexRunner.content.includes("!isPostResearchResume")
  || !sourceFiles.codexRunner.content.includes("Image Worker 실행 준비")
  || !sourceFiles.codexRunner.content.includes("Writer Agent 실행 준비")
  || !sourceFiles.codexRunner.content.includes("Main Agent 최종 검수 실행 준비")
  || !sourceFiles.codexRunner.content.includes("resumeNeedsWriterRepair")
  || !sourceFiles.codexRunner.content.includes("저장된 본문을 Writer가 먼저 수정한 뒤 Main 검수를 재개합니다")
  || !sourceFiles.codexRunner.content.includes("DEFAULT_CODEX_TASK_TIMEOUT_MS = 10 * 60 * 1000")
  || !sourceFiles.codexRunner.content.includes("image-worker-progress-<sequence>.json")
  || !sourceFiles.main.content.includes("readImageProgressArtifacts")
  || !sourceFiles.codexRunner.content.includes("resumeState?.imageWorkerResult")
) {
  failed = true;
  console.error("src/lib/codexRunner.js: generation phases must checkpoint their next resumable stage");
}
if (
  !sourceFiles.rendererApp.content.includes("function showResumePanel")
  || !sourceFiles.rendererApp.content.includes("#resumePromptInput")
  || !sourceFiles.rendererApp.content.includes("window.blogAuto.resumeJob({ jobId, repairPrompt })")
  || !sourceFiles.rendererApp.content.includes("오류 해결 프롬프트(선택) 값을 입력하여 재개 중")
) {
  failed = true;
  console.error("src/renderer/app.js: failed jobs must expose a repair-prompt resume action");
}
if (
  !sourceFiles.recoveryPlaybook.content.includes("function recordRecoveryLesson")
  || !sourceFiles.recoveryPlaybook.content.includes("function findRecoveryGuidance")
  || !sourceFiles.recoveryPlaybook.content.includes("function recordRecoveryAttempt")
  || !sourceFiles.recoveryPlaybook.content.includes("function recoveryOverview")
  || !sourceFiles.recoveryPlaybook.content.includes("recovery-playbook.json")
  || !sourceFiles.main.content.includes("recordRecoveryLesson(runtimeRoot")
  || !sourceFiles.main.content.includes("findRecoveryGuidance(runtimeRoot")
  || !sourceFiles.main.content.includes('ipcMain.handle("recovery:overview"')
  || !sourceFiles.main.content.includes('ipcMain.handle("recovery:recordAttempt"')
  || !sourceFiles.codexRunner.content.includes("Previously successful recovery guidance")
) {
  failed = true;
  console.error("recovery playbook: successful repair prompts must be persisted and reused as advisory guidance");
}
if (
  !sourceFiles.rendererIndex.content.includes("id=\"openRecoveryGuideButton\"")
  || !sourceFiles.rendererIndex.content.includes("오류 유형 및 적용한 해결 방향")
  || !sourceFiles.rendererIndex.content.includes("id=\"recoveryGuideModal\"")
  || !sourceFiles.rendererIndex.content.includes("id=\"recoveryGuideTableBody\"")
  || !sourceFiles.rendererApp.content.includes("#openRecoveryGuideButton")
  || !sourceFiles.rendererApp.content.includes("#closeRecoveryGuideButton")
  || !sourceFiles.rendererApp.content.includes("function renderRecoveryGuide")
  || !sourceFiles.rendererApp.content.includes("recordRecoveryAttempt")
  || !sourceFiles.preload.content.includes("getRecoveryOverview")
  || !sourceFiles.preload.content.includes("recordRecoveryAttempt")
  || !sourceFiles.rendererStyles.content.includes(".recovery-guide-table")
) {
  failed = true;
  console.error("recovery guide UI: the resume panel must expose the enlarged error-type and resolution popup");
}
if (
  !analyzeSelectedHistoryBlock
  || !analyzeSelectedHistoryBlock.content.includes("const queue =")
  || !analyzeSelectedHistoryBlock.content.includes("다음 선택 항목으로 계속 진행합니다")
  || !analyzeSelectedHistoryBlock.content.includes("continue;")
  || analyzeSelectedHistoryBlock.content.includes("해당 지점에서 일시중지합니다")
) {
  failed = true;
  console.error("src/renderer/app.js: bulk history diagnosis must continue with the next selected item after an individual failure");
}
if (!publishApprovedDailyBlock || publishApprovedDailyBlock.content.includes("publishMemberBoardPost")) {
  failed = true;
  console.error("src/main.js: approval and 12:00 scheduled publishing must remain Naver-only");
}
if (!randomDailyResearchBlock || !randomDailyResearchBlock.content.includes("publishMemberBoardPost")) {
  failed = true;
  console.error("src/main.js: random topic publishing must remain the only OnsaeCar registration entry point");
}
if (
  !publishHistoryDraftBlock
  || !publishHistoryDraftBlock.content.includes("expectedTitle")
  || !publishHistoryDraftBlock.content.includes("선택 큐 항목 불일치")
  || !publishHistoryDraftBlock.content.includes("queuePosition")
) {
  failed = true;
  console.error("src/main.js: history publishing must validate the requested queue title before publishing");
}
if (
  !publishHistoryDraftsBlock
  || !publishHistoryDraftsBlock.content.includes("const uniqueJobIds")
  || !publishHistoryDraftsBlock.content.includes("for (let index = 0; index < uniqueJobIds.length; index += 1)")
  || !publishHistoryDraftsBlock.content.includes("expectedTitles[selectedJobId]")
  || !publishHistoryDraftsBlock.content.includes("queuePosition: index + 1")
) {
  failed = true;
  console.error("src/main.js: bulk history publishing must process an immutable ordered job ID queue");
}
if (
  !publishSelectedHistoryBlock
  || !publishSelectedHistoryBlock.content.includes("const queue = selectedItems.map")
  || !publishSelectedHistoryBlock.content.includes("expectedTitles: Object.fromEntries")
  || !publishSelectedHistoryBlock.content.includes("jobIds: queue.map")
  || !publishSelectedHistoryBlock.content.includes("각 항목의 ID·제목을 고정")
) {
  failed = true;
  console.error("src/renderer/app.js: bulk history publishing must snapshot each selected job ID and title before starting");
}
